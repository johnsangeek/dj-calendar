...
import { Home, Users, FileText, Settings, Bell, X, ExternalLink, BarChart3 } from 'lucide-react';
...
export const WebNav = () => {
  ...
  const navLinks = [
    { href: '/web', label: 'Dashboard', icon: Home },
    { href: '/web/clients', label: 'Clients', icon: Users },
    { href: '/crm', label: 'CRM', icon: BarChart3 },
    { href: '/web/invoices', label: 'Factures', icon: FileText },
    { href: '/web/settings', label: 'Paramètres', icon: Settings },
  ];

  const isActive = (href: string) => {
    if (href === '/web') return pathname === '/web';
                    ...
                    {pendingBookings.length === 0 ? (
                      ...
                      <div className="divide-y divide-gray-50">
                        {pendingBookings.map(({ booking, daysSince }) => (
                          <Link
                            key={booking.id}
                            href={`/web/invoices?booking=${booking.id}`}
                            onClick={() => setShowNotifications(false)}
